...
public class BaseModel {
    ...
    public <B extends BaseBean> void upload(String api, final Class<B> clazz) {
        ...
        if (responseBodySubscriber == null) return;
        if (mBuilder.mBaseRequestView instanceof FragmentActivity) {
            mHttpRequest.uploadFile(api, mBuilder.mParam, mBuilder.multiFileKey, mBuilder.imgFormat, mBuilder.mFileInfoBeans).to(autoDisposable(AndroidLifecycleScopeProvider.from(((FragmentActivity) mBuilder.mBaseRequestView)))).subscribe(responseBodySubscriber);
        } else if (mBuilder.mBaseRequestView instanceof Fragment) {
            mHttpRequest.uploadFile(api, mBuilder.mParam, mBuilder.multiFileKey, mBuilder.imgFormat, mBuilder.mFileInfoBeans).to(autoDisposable(AndroidLifecycleScopeProvider.from(((Fragment) mBuilder.mBaseRequestView)))).subscribe(responseBodySubscriber);
        } else
            mHttpRequest.uploadFile(api, mBuilder.mParam, mBuilder.multiFileKey, mBuilder.imgFormat, mBuilder.mFileInfoBeans).subscribe(responseBodySubscriber);
    }
    ...
    public static class Builder {
        ...
        //
        private String imgFormat;//指定文件上传图片格式,仅适用于文件上传(实际文件未变，只是传上去的图片格式变了)
        ...
        public Builder setImgFormat(String format) {
            imgFormat = format;
            return this;
        }
